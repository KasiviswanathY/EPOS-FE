...
export const createLocations = createAsyncThunk(
  ...
  async ({ payload, token }: any, thunkAPI) => {
    try {
      const response = await axios.post(apiRoutes.locations, payload, {
        headers: {
...
export const getLocations = createAsyncThunk(
  ...
  async ({ token }: { token: string }, thunkAPI) => {
    try {
      const res = await axios.get(apiRoutes.locations, {
...
export const deleteLocation = createAsyncThunk(
  ...
  async ({ id, token }: any, thunkAPI) => {
    try {
      const response = await axios.delete(`${apiRoutes.locations}/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
