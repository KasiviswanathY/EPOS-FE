...
export default function CustomerDetailsPage({
  ...
  const [newMember, setNewMember] = useState<AdditionalMember>({
    firstName: "",
    lastName: "",
    dob: "",
  });
  ...
  const DetailRow = ({
    ...
    <div className="row mb-2">
      <label className="col-sm-2 col-form-label text-end fw-semibold">
        {label}:
      </label>
      ...
      <div className="content">
        ...
        <div className="card mb-4">
          <div className="card-body">
            ...
            <DetailRow
              label="Main Address"
              value={customer.mainAddress || "No Main Address"}
            />
            ...
            <DetailRow
              label="Type"
              value={customer.type || "No Customer Type"}
            />
            <DetailRow
              label="Max Credit"
              value={`$${customer.maxCredit.toFixed(2)}`}
            />
            <DetailRow
            ...
            <DetailRow
              label="Sign Up Date"
              value={new Date(customer.signUpDate).toLocaleString()}
            />
            <DetailRow label="Notes" value={customer.notes} />
            <DetailRow
              label="Sign Up Location"
              value={customer.signUpLocation}
            />
          </div>
        </div>
        <div className="card mb-4">
            ...
            {customer.additionalMembers.length === 0 ? (
              ...
              customer.additionalMembers.map((m, i) => (
                <DetailRow
                  key={i}
                  label={`Member ${i + 1}`}
                  value={`${m.firstName} ${m.lastName} (DOB: ${m.dob})`}
                />
              ))
            )}
        ...
        <div className="card mb-4">
          <div className="card-body">
            <h5 className="fw-bold mb-3">Customers Addresses</h5>
            ...
            {["name", "address1", "address2", "town", "state", "zip"].map(
              (field) => (
